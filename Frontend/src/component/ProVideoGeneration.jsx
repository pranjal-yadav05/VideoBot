...
const ProVideoGeneration = () => {
  ...
  useEffect(() => {
    ...
    const fetchPreviousVideos = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`${API_URL}/pro-videos`);
    ...
    <div className="min-h-screen bg-black text-gray-300 relative overflow-hidden flex">
      ...
      <div className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'ml-72' : 'ml-0'}`}>
        ...
        <div className="container mx-auto min-h-screen flex flex-col items-center justify-center px-4">
          ...
          <p className="mt-4 text-gray-400 text-sm text-center border-t border-gray-700 pt-3">
            <span className="font-semibold text-gray-300">VidBot Pro</span> generates dynamic animated educational videos with AI-powered narration. 
            <br className="hidden sm:block" /> While the audio may not be perfectly synced with the visuals, it still enhances the learning experience.
          </p>
